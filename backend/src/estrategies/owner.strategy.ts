...
export class OwnerStrategy implements AuthenticationStrategy {
  name: string = 'owner';
  ...
  authenticate(request: Request<ParamsDictionary, any, any, ParsedQs, Record<string, any>>): Promise<UserProfile | RedirectRoute | undefined> {
    ...
    console.log("decoooodeeeed", decodedToken)
    const profileData = this.strategyService.autheticate(decodedToken, Rol.Owner);
