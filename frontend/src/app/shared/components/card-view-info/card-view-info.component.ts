...
@Component({
  ...
  template: `
    <ion-card>
      <ion-card-content>
        <ion-list lines="none">
          @for (column of columnsData; track column.key) {
            @if (column.key !== 'actions') {
              <ion-item>
                <ion-label>{{ column.alias }}</ion-label>
                <ng-container [ngSwitch]="column.type">
                  <ion-badge *ngSwitchCase="'boolean'" slot="end" [color]="getCellValue(element, column) ? 'success' : 'medium'">
                    {{ getCellValue(element, column) ? "Activo" : "Inactivo" }}
                  </ion-badge>
                  <ion-note *ngSwitchCase="'object'" slot="end">{{ getCellValue(element, column) }}</ion-note>
                  <ion-note *ngSwitchCase="'date'" slot="end">{{ getCellValue(element, column) | date:'dd/MM/yyyy' }}</ion-note>
                  <ion-note *ngSwitchCase="'dni'" slot="end">{{ getCellValue(element, column) | formatDni }}</ion-note>
                  <ng-container *ngSwitchCase="'xml'">
                    <ion-list>
                      @for (item of getCellValue(element, column) | xmlToList; track item) {
                        <ion-item lines="none">
                          <ion-label>{{ item }}</ion-label>
                        </ion-item>
                      } @empty {
                        <ion-item lines="none">
                          <ion-label color="medium">No existen elementos para mostrar.</ion-label>
                        </ion-item>
                      }
                    </ion-list>
                  </ng-container>
                  <ion-note *ngSwitchDefault slot="end">{{ getCellValue(element, column) }}</ion-note>
                </ng-container>
              </ion-item>
            }
          }
  ...
  getCellValue(row: any, column: Column): any {
    ...
    if (column.combineWith) {
      ...
      return column.combineFormat 
        ...
        : `${this.formatValue(primaryValue)} ${this.formatValue(secondaryValue)}`;
    }
